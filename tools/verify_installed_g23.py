...
def main() -> int:
    if len(SCHEMA_NAMES) != 27:
        raise SystemExit(f"installed G23 schema inventory is {len(SCHEMA_NAMES)}, expected 27")
    with tempfile.TemporaryDirectory(prefix="gigai-g23-installed-") as directory:
        ...
        manifest_bytes = canonical_json_bytes(_manifest(digest_imported_bytes(source)))
