...
3. **Explicit safe migration — Pass.** Tests cover successful and idempotent v1-to-v2 migration, an interrupted atomic replacement that leaves the v1 bytes intact, malformed and unknown versions, ambiguous state, and a secret canary. A later explicit `gigai setup` rerun performs the migration; no automatic resume is attempted. Configuration persists references only; the canary cannot enter saved configuration or diagnostics.
