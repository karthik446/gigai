...
2. Set `pyproject.toml [project].version` to `0.1.3`, run `uv lock`, and verify
   `uv lock --locked`.
3. Update user-facing installation documentation for `uv tool install
   "gigai==0.1.3"`; do not alter historical Phase 1 evidence.
4. Commit the release change, create an annotated `v0.1.3` tag, and push the
   commit and tag.
