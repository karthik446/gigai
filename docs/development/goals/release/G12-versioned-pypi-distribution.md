...
Deliberately publish GigAI `0.1.2` as the first production pre-alpha release: an
...
The package metadata, annotated Git tag `v0.1.2`, PyPI release, GitHub Release,
and `gigai --version` must all identify the same version. This explicitly
declares the first production PyPI release and activates ADR 0003's
immutable/additive schema-versioning regime.
...
- Clean-machine operator evidence on one supported macOS machine and one
  ...
  `uv tool install "gigai==0.1.2"`, then proves `gigai --version`,
...
1. Before creating the release commit or annotated `v0.1.2` tag, GigAI's
...
2. The release commit sets the static `pyproject.toml [project].version` to
   `0.1.2` and regenerates `uv.lock` with `uv lock`. At the exact annotated
   `v0.1.2` tag commit, the lockfile project entry, built-artifact metadata,
...
8. Sanitized clean-machine macOS and Linux proofs install the exact PyPI
   version with `uv tool install "gigai==0.1.2"` and pass the stated offline
