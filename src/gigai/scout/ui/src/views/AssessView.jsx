...
// P5/P9 → Q4a-nav: "+ Assess a job" (#/assess), reached from the Jobs page
// instead of a tab. POST /api/assess -- URL or pasted text, a profile or a
...
//
// The result is no longer rendered here: `onAssessed(response)` hands the
// AssessResponse to the app, which merges it into the job model and opens
// its job page (#/jobs/<job_identity>, the same JobPage every card opens),
// so an on-demand assessment reads exactly like a run's.
export default function AssessView({ profiles, selectedProfileId, onAssessed }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const resume = resumeMode === "profile" ? { profile_id: profileId || null } : { resume_text: resumeText };
  ...
  // "Selected profile (default)" is a valid choice: POST /api/assess takes
  // profile_id null as "the selected profile" (the old panel disabled the
  // button for it, which left a fresh page with nothing to submit).
  const canSubmit = Boolean(jobMode === "url" ? jobUrl.trim() : jobText.trim()) && (resumeMode === "profile" || Boolean(resumeText.trim()));

  return (
    <div>
      <Breadcrumb crumbs={[{ label: "Jobs", href: JOBS_HASH }, { label: "Assess a job" }]} />
      <div className="panel">
        <h2>Assess a job</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            <div className="toggle-row">
              ...
              <button type="button" className={`toggle-button${resumeMode === "text" ? " active" : ""}`} onClick={() => setResumeMode("text")}>
                Paste resume text
              </button>
            </div>
          </div>
          ...
          <div className="actions" style={{ justifyContent: "flex-start" }}>
            ...
            <a className="button secondary" href={JOBS_HASH}>
              Cancel
            </a>
          </div>
