...
SOURCES: the RESUME LINES are numbered R1, R2, ... (the number is the line's position in the candidate's resume; a copy or a citation refers to that line only). A resume sentence may wrap across several numbered lines (R4 ends mid-sentence, R5 finishes it): when a fact spans several numbered lines, cite every line it spans. The ANSWERS, when present, are facts the candidate stated in earlier GigAI questions, each keyed by its question id (cite one as {"kind": "answer", "question_id": "<id>"}). The REQUIREMENT MATRIX, when present, is GigAI's own earlier assessment of the posting: it tells you which requirements matter and which the candidate met; it is context only and can never be cited as a source.
...
FRAMING: keep the source's framing of the candidate's role. Never upgrade participation into initiation or ownership: a source that says the work came "with SLOs" does not support "introducing SLOs"; "worked on the Kubernetes migration" does not support "led the Kubernetes migration"; "part of the payments team" does not support "owned payments". Not allowed: R7 "Worked on the Kubernetes migration with the platform team" rewritten as "Led the Kubernetes migration". Allowed: the same R7 rewritten as "Worked on the platform team's Kubernetes migration", which tightens the line and keeps the candidate's role exactly as stated. PRECISION: keep the source's modality and qualifiers when you paraphrase: "auditable" is not "audited", "comfortable owning" is not "owned", and a qualifier stays on the clause it qualifies (monitors that "caught problems before they reached dashboards" do not make the candidate's investigation happen before they reached dashboards).
