...
from .proposals import ScoutProposalRequest, validate_proposal_output
from .find_jobs.contracts import AssessmentResult, PinnedResume, Producer, SelectedPosting
from ..validators import validate_model_invocation, validate_serialized_contract
from .proposal_execution import ScoutProposalExecution, _resolve_sources
from .inputs import resolve_external_input, _record_revision
...
_MAX_BYTES = 512 * 1024
_MAX_INPUTS = 12
_ASSESSMENT_SCHEMA = "scout-assessment-revision:1"
...
def _validate_record(value: object) -> dict[str, object]:
    ...
    if value.get("schema_version") == _ASSESSMENT_SCHEMA:
        required = {"schema_version", "record_id", "revision_id", "project_id", "gig_id", "state", "assessment", "pinned_resume", "producer", "created_at"}
        if set(value) != required or value.get("state") != "active":
            _fail("proposal_record_invalid", "assessment record fields are not closed")
        _id(value.get("record_id"), "record")
        _id(value.get("revision_id"), "revision")
        if not isinstance(value.get("project_id"), str) or not isinstance(value.get("gig_id"), str):
            _fail("proposal_record_invalid", "assessment record scope is invalid")
        try:
            assessment = AssessmentResult.from_json(value.get("assessment"))
            PinnedResume.from_json(value.get("pinned_resume"))
            Producer.from_json(value.get("producer"))
        except Exception as exc:
            _fail("proposal_record_invalid", "assessment record payload is invalid")
            raise AssertionError from exc
        if not isinstance(value.get("created_at"), str) or not value["created_at"]:
            _fail("proposal_record_invalid", "assessment record timestamp is invalid")
        return {**dict(value), "assessment": assessment.to_json()}
...
def save_assessment_revision(*, home_root, target, posting: SelectedPosting, result: AssessmentResult, producer: Producer, pinned_resume: PinnedResume) -> str:
    """Persist a frozen find-jobs assessment revision and return its artifact ref."""
    if not isinstance(posting, SelectedPosting) or not isinstance(result, AssessmentResult) or not isinstance(producer, Producer) or not isinstance(pinned_resume, PinnedResume):
        _fail("proposal_record_invalid", "assessment revision inputs are invalid")
    if result.posting != posting:
        _fail("proposal_record_invalid", "assessment posting differs from selected posting")
    from ..workpad import resolve_workpad
    resolved = target if isinstance(target, ResolvedWorkpad) else resolve_workpad(home_root=home_root, requested_target=target, gig_id=None, allow_semantic_state=True)
    record_id = f"record_{uuid.uuid4()}"
    revision_id = f"revision_{uuid.uuid4()}"
    path = f"records/scout-proposals/{record_id}/revisions/{revision_id}.json"
    record = _validate_record({
        "schema_version": _ASSESSMENT_SCHEMA, "record_id": record_id, "revision_id": revision_id,
        "project_id": resolved.project_id, "gig_id": resolved.gig_id, "state": "active",
        "assessment": result.to_json(), "pinned_resume": pinned_resume.to_json(),
        "producer": producer.to_json(), "created_at": _now(),
    })
    data = canonical_json_bytes(record)
...
__all__ = ["AnswerAssociationResult", "ProposalRevisionResult", "ScoutProposalRecordError", "answer_association_for_source", "read_proposal_revision", "record_proposal_revision", "save_assessment_revision", "validate_proposal_revision"]
