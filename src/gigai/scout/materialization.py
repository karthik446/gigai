...
from .template import (SCOUT_DEFINITION_VERSION, SCOUT_DISCOVERY_SCHEMA_PATH,
                              ...
                              scout_source_files)
from .bundled_tools import SCOUT_CRUD_MANIFEST_ID, prepared_scout_crud_manifest
from .find_jobs.contracts import (ACQUIRE_CAPABILITY, ACQUIRE_DECLARED_EFFECTS,
                                         ASSESS_CAPABILITY, ASSESS_DECLARED_EFFECTS,
                                         PRESENT_CAPABILITY, PRESENT_DECLARED_EFFECTS,
                                         ModelTarget)
...
# Amendment 02 (D11/T1): sealed functional graph name for the documentation's
# `find-jobs:functional:1` — the descriptor `graph_id` pattern forbids colons,
# so this selector carries that identity into the schema-valid string space.
# It is a distinct descriptor from the historical inert `find-jobs` selector
# in ``SCOUT_GRAPHS``; both are compiled into the same Graph Set.
_FIND_JOBS_FUNCTIONAL_SELECTOR = "find-jobs-functional"

_FIND_JOBS_FUNCTIONAL_GOALS = (
    ("acquire", "Acquire postings", ACQUIRE_CAPABILITY, ACQUIRE_DECLARED_EFFECTS),
    ("assess", "Assess postings", ASSESS_CAPABILITY, ASSESS_DECLARED_EFFECTS),
    ("present", "Present results", PRESENT_CAPABILITY, PRESENT_DECLARED_EFFECTS),
)

# One aggregate budget, sized to bound three Goals (each below), not the
# single-Goal ``_BUDGET`` the other five compiled selectors use unmodified.
_FIND_JOBS_FUNCTIONAL_GOAL_BUDGET = {
    "max_model_calls": 1,
    "max_tool_calls": 0,
    "max_tokens": 4000,
    "max_cost": "0.16",
...
_FIND_JOBS_FUNCTIONAL_AGGREGATE_BUDGET = {
    "max_model_calls": 3,
    "max_tool_calls": 0,
    "max_tokens": 12000,
    "max_cost": "0.50",
...
def _find_jobs_functional_graph_and_descriptor(
    *, gig_id: str, created_at: str, uuid_factory: Callable[[], uuid.UUID]
) -> tuple[dict[str, object], dict[str, bytes], tuple[str, ...]]:
    """Author the sealed ``find-jobs:functional:1`` graph (Amendment 02, I-1).

    Three bound goals (acquire, assess, present) with two automatic
    ``COMPLETE`` dependency edges and no branch/recovery/custom outcome, per
    P2-FREEZE-04 Amendment 02 D11 and the find-jobs-functional roadmap's I-1
    row.  Returns the graph-set descriptor, the compiled artifact bytes to
    merge into the Graph Set's ``compiled`` mapping, and the new goal IDs
    (for the CRUD manifest scope).
    """

    contracts_root = f"compiled/{_FIND_JOBS_FUNCTIONAL_SELECTOR}"
    definition_path = f"{_FIND_JOBS_FUNCTIONAL_SELECTOR}/goal-graph.json"
    graph_id = _id(EntityPrefix.GRAPH, uuid_factory)
    goal_ids = [_id(EntityPrefix.GOAL, uuid_factory) for _ in _FIND_JOBS_FUNCTIONAL_GOALS]
    goals: list[dict[str, object]] = []
    compiled: dict[str, bytes] = {}
    all_effects: set[str] = set()
    all_capabilities: set[str] = set()
    all_completion_evidence = [f"{slug}-completion" for slug, _title, _capability, _effects in _FIND_JOBS_FUNCTIONAL_GOALS]
    for ordinal, (goal_id, (slug, title, capability, effects)) in enumerate(zip(goal_ids, _FIND_JOBS_FUNCTIONAL_GOALS)):
        effects_sorted = sorted(effects)
        all_effects.update(effects_sorted)
        all_capabilities.add(capability)
        instructions = f"# {title}\n\nSealed find-jobs:functional:1 {slug} goal.\n".encode("utf-8")
        compiled_goal_contract = f"{contracts_root}/{slug}/goal-contract.md"
        compiled[compiled_goal_contract] = instructions
        goal_contract = _artifact_ref(f"{_FIND_JOBS_FUNCTIONAL_SELECTOR}/{slug}/goal-contract.md", "text/markdown", instructions)
        # The terminal goal (present) must require the Gig's full completion
        # evidence set; the others require only their own (validators.py
        # ``incomplete_terminal_evidence``/D6-adjacent completion semantics).
        required_evidence = all_completion_evidence if slug == "present" else [f"{slug}-completion"]
        goals.append({
            "goal_id": goal_id,
            "goal_version": 1,
            ...
            "executor": {"kind": "local_capability", "capability": capability, "role": None, "resolution": "installed", "materialized_by": None, "blocking_reason": None},
            "tools": [],
            "effects": effects_sorted,
            "write_surfaces": ["docs/", "runs/"],
            "exclusive_resources": ["scout-candidate"],
            "budget": dict(_FIND_JOBS_FUNCTIONAL_GOAL_BUDGET),
            "verification": {"verifier": "gigai.check", "acceptance": "Required outputs and declared evidence are present.", "required_evidence": required_evidence},
            "outcomes": ["COMPLETE"],
        })
    edges = [
        {
            "edge_id": _id(EntityPrefix.EDGE, uuid_factory),
            "from_goal_id": goal_ids[0],
            "to_goal_id": goal_ids[1],
            "kind": "dependency",
        ...
        {
            "edge_id": _id(EntityPrefix.EDGE, uuid_factory),
            "from_goal_id": goal_ids[1],
            "to_goal_id": goal_ids[2],
            "kind": "dependency",
            "on_outcomes": ["COMPLETE"],
            "automatic": True,
        },
    ...
    graph = {
        ...
        "graph_version": 1,
        "created_at": created_at,
        "aggregate_budget": dict(_FIND_JOBS_FUNCTIONAL_AGGREGATE_BUDGET),
        "failure_policy": "fail_gig",
        "goals": goals,
        "edges": edges,
        "entry_goal_ids": [goal_ids[0]],
        "terminal_goal_ids": [goal_ids[2]],
        "required_completion_evidence": ["acquire-completion", "assess-completion", "present-completion"],
    }
    graph_data = canonical_json_bytes(graph)
    compiled[f"{contracts_root}/goal-graph.json"] = graph_data
    descriptor: dict[str, object] = {
        "graph_id": _FIND_JOBS_FUNCTIONAL_SELECTOR,
        "purpose": "Sealed functional find-jobs traversal: acquire, assess, present.",
        "aliases": [],
        "routing_summary": "Find jobs (functional)",
        "goal_graph": _artifact_ref(definition_path, "application/json", graph_data),
        "effect_policy": sorted(all_effects),
        "capability_requirements": sorted(all_capabilities),
        "provider_eligibility": {"providers": sorted({ModelTarget.OLLAMA_LOCAL.value, ModelTarget.CODEX_CLI.value, ModelTarget.OPENROUTER_API.value})},
        "budget": dict(_FIND_JOBS_FUNCTIONAL_AGGREGATE_BUDGET),
    }
    contract_values = {
        "input_contract": {"schema_version": "1.0", "kind": "run_input_contract", "gig_id": gig_id, "fields": ["config", "config_digest", "selection_cap", "selection_rule", "pinned_resume", "model_target"]},
        "output_contract": {"schema_version": "1.0", "kind": "run_output_contract", "gig_id": gig_id, "fields": ["discovery"]},
        ...
        "completion_evidence_contract": {"schema_version": "1.0", "kind": "completion_evidence_contract", "gig_id": gig_id, "fields": ["acquire-completion", "assess-completion", "present-completion"]},
    }
    for field, value in contract_values.items():
        path = f"{contracts_root}/{field}.json"
        data = canonical_json_bytes(value)
        compiled[path] = data
        descriptor[field] = _artifact_ref(f"{_FIND_JOBS_FUNCTIONAL_SELECTOR}/{field}.json", "application/json", data)
    ...
    descriptor["review_contract"] = _artifact_ref(f"{_FIND_JOBS_FUNCTIONAL_SELECTOR}/review-contract.json", "application/json", review)
    return descriptor, compiled, tuple(goal_ids)
...
def _compiled_snapshot(
    *, gig_id: str, source: Mapping[str, bytes], uuid_factory: Callable[[], uuid.UUID]
    ...
    find_jobs_functional_descriptor, find_jobs_functional_compiled, find_jobs_functional_goal_ids = (
        _find_jobs_functional_graph_and_descriptor(
            gig_id=gig_id, created_at=created_at, uuid_factory=uuid_factory
        )
    )
    compiled.update(find_jobs_functional_compiled)
    descriptors.append(find_jobs_functional_descriptor)
    goal_ids.extend(find_jobs_functional_goal_ids)
    ...
    # The shared policy is one Gig-wide ceiling; every existing descriptor's
    # own effect_policy/capability_requirements/provider_eligibility/budget
    # (each unchanged: ["write_workpad"]/["gigai.offline"]/["deterministic"]/
    # ``_BUDGET``) remains well within it, so this widening does not alter
    # their compiled bytes.
    shared_effects = sorted({"write_workpad", *ACQUIRE_DECLARED_EFFECTS, *ASSESS_DECLARED_EFFECTS, *PRESENT_DECLARED_EFFECTS})
    shared_capabilities = sorted({"gigai.offline", ACQUIRE_CAPABILITY, ASSESS_CAPABILITY, PRESENT_CAPABILITY})
    shared_providers = sorted({"deterministic", ModelTarget.OLLAMA_LOCAL.value, ModelTarget.CODEX_CLI.value, ModelTarget.OPENROUTER_API.value})
    shared_budget = {
        "max_model_calls": max(_BUDGET["max_model_calls"], _FIND_JOBS_FUNCTIONAL_AGGREGATE_BUDGET["max_model_calls"]),
        "max_tool_calls": max(_BUDGET["max_tool_calls"], _FIND_JOBS_FUNCTIONAL_AGGREGATE_BUDGET["max_tool_calls"]),
        "max_tokens": max(_BUDGET["max_tokens"], _FIND_JOBS_FUNCTIONAL_AGGREGATE_BUDGET["max_tokens"]),
        "max_cost": max(_BUDGET["max_cost"], _FIND_JOBS_FUNCTIONAL_AGGREGATE_BUDGET["max_cost"], key=float),
        "currency": _BUDGET["currency"],
        "max_wall_time_ms": max(_BUDGET["max_wall_time_ms"], _FIND_JOBS_FUNCTIONAL_AGGREGATE_BUDGET["max_wall_time_ms"]),
        "max_parallel_goals": max(_BUDGET["max_parallel_goals"], _FIND_JOBS_FUNCTIONAL_AGGREGATE_BUDGET["max_parallel_goals"]),
    }
    definition = {
        "schema_version": "1.0",
        "gig_id": gig_id,
        ...
        "graphs": descriptors,
        "shared_policy": {"effects": shared_effects, "required_capability_ids": shared_capabilities, "provider_eligibility": {"providers": shared_providers}, "budget": shared_budget},
    }
