...
def test_installed_help_version_and_goal_approved_commands_are_the_only_surface(
    ...
    assert "gigs" in help_result.stdout
    assert "proposals" in help_result.stdout
    assert "status" in help_result.stdout
    assert "show" in help_result.stdout
    assert "history" in help_result.stdout
    ...
    assert (
            "Choose 'setup', 'doctor', 'init', 'create', 'improve', 'feedback', 'revise', 'approve', 'reject', 'gigs', 'proposals', 'status', 'show', 'history', 'plan', 'run', 'run-details', 'occurrence', 'workpad', 'check', 'models', 'invoke', 'eval', or 'open'"
