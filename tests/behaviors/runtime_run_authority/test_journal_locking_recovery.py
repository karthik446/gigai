...
def test_first_transition_commits_g05_infrastructure_and_canonical_handoff(
    tmp_path: Path,
) -> None:
    ...
    assert (
        ...
        in _git(workpad, "show", "--format=%B", "--no-patch", "HEAD").stdout
    )
    ...
    assert (
        resolve_workpad(home_root=home, requested_target=target, gig_id=GIG_ID).path
        == workpad
    )


def test_normal_sequence_reads_head_and_rejects_an_uncommitted_next_handoff(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(RuntimeError, match="after_replace"):
        _write(
            workpad,
            3,
            observer=lambda step: (_ for _ in ()).throw(RuntimeError(step))
    ...
    with pytest.raises(JournalReconciliationRequired, match="uncommitted"):
        record_transition(
            ...
            project_id=PROJECT_ID,
            gig_id=GIG_ID,
            handoff_id=_handoff(4),
            transition="creation_started",
            body="next",
        )


def test_artifacts_and_handoff_share_one_semantic_commit(tmp_path: Path) -> None:
    from gigai.journal import JournalArtifact
    ...
    entry = _write(
        ...
        1,
        artifacts=(
            JournalArtifact("gig.md", b"# Gig\n"),
            JournalArtifact("manifests/example.json", b'{"example":true}'),
        ),
    )

    assert _git(
        workpad, "show", "--format=", "--name-only", entry.commit
    ).stdout.splitlines() == [
        ".gitignore",
        "gig.md",
        "handoffs/000000000001-creation-started.txt",
...
def test_artifact_transaction_recovery_commits_exact_declared_bytes_once(
    ...
    from gigai.journal import JournalArtifact, reconcile_journal
    ...
    artifacts = (JournalArtifact("gig.md", b"# Recovered Gig\n"),)
    with pytest.raises(RuntimeError, match="after_artifact_replace"):
        _write(workpad, 1, artifacts=artifacts, observer=crash)

    transaction = workpad / "scratch" / ".gigai-journal-000000000001.json"
    assert transaction.is_file()
    assert (workpad / "gig.md").read_bytes() == b"# Recovered Gig\n"
    recovered = reconcile_journal(workpad=workpad, project_id=PROJECT_ID, gig_id=GIG_ID)

    assert recovered.reconciled is True
    assert recovered.sequence == 1
    assert not transaction.exists()
    assert _git(workpad, "rev-list", "--count", "HEAD").stdout.strip() == "1"
    assert _git(workpad, "show", "HEAD:gig.md").stdout == "# Recovered Gig\n"


def test_chained_transitions_hold_ordered_entries_under_one_writer_lock(
    tmp_path: Path,
) -> None:
    from gigai.journal import JournalTransition, record_transition_chain

    workpad = _workpad(tmp_path)

    first, second = record_transition_chain(
        workpad=workpad,
        project_id=PROJECT_ID,
        gig_id=GIG_ID,
        first=JournalTransition(_handoff(1), "creation_started", "started"),
        continuation=lambda entry: JournalTransition(
            _handoff(2),
            "gig_accepted",
            f"sealed {entry.commit}",
        ),
    )

    assert (first.sequence, second.sequence) == (1, 2)
    assert _git(workpad, "rev-list", "--count", "HEAD").stdout.strip() == "2"
    assert first.commit in second.path.read_text(encoding="utf-8")


@pytest.mark.parametrize(
    "failpoint", ("before_replace", "after_replace", "before_commit", "after_commit")
)
def test_explicit_recovery_converges_crash_boundaries(
    ...
    if failpoint == "before_replace":
        ...
        assert (
            _git(workpad, "rev-parse", "--verify", "HEAD", check=False).returncode != 0
        )
        _write(workpad, 1)
    else:
        result = reconcile_journal(
...
def test_later_transition_crash_recovery_never_duplicates_sequence(
    tmp_path: Path, failpoint: str
) -> None:
    from gigai.journal import reconcile_journal
    ...
    assert result.reconciled is (
        failpoint != "before_replace" and failpoint != "after_commit"
    )
...
def test_eight_process_race_allocates_strict_committed_order(tmp_path: Path) -> None:
    ...
    processes = [
        context.Process(target=_worker, args=(os.fspath(workpad), index))
...
def test_writer_lock_retries_with_a_nonzero_backoff(
...
def test_failed_mount_probe_blocks_before_handoff_mutation(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _root: (
            DiagnosticCheck(
                "mount.atomic_replace", "mount", "FAIL", "injected", (), None, 0
            ),
        ),
    )
...
def test_mount_change_and_divergent_head_require_operator_recovery(
    tmp_path: Path,
) -> None:
...
def test_journal_module_never_allocates_project_or_gig_or_provisions_workpad() -> None:
    path = Path(__file__).parents[3] / "src" / "gigai" / "journal.py"
    ...
    record = next(
        ...
        if isinstance(node, ast.FunctionDef) and node.name == "record_transition"
    )
    methods = {
        node.attr for node in ast.walk(record) if isinstance(node, ast.Attribute)
    }
