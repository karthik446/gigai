...
def test_g17_additive_schema_inventory_and_baseline_hashes() -> None:
    ...
    root = Path(__file__).parents[3] / "src/gigai/schemas"
...
def test_capability_manifest_is_linked_by_the_g15_bundle(tmp_path: Path) -> None:
    from tests.behaviors.runtime_run_authority.test_g15_review_substrate import _bundle
...
def test_capability_module_has_no_effectful_imports() -> None:
    tree = ast.parse(Path(__file__).parents[3].joinpath("src/gigai/capabilities.py").read_text())
    imported = {
